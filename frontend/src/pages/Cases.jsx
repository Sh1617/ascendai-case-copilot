...
import { Search, Filter, Plus, ArrowUpDown, ChevronRight } from 'lucide-react';
...
export default function CasesPage() {
  ...
  const priorityColor = { Urgent: 'text-red-600 bg-red-50', High: 'text-amber-600 bg-amber-50', Medium: 'text-brand-600 bg-brand-50', Low: 'text-slate-500 bg-slate-100' };
      ...
      <div className="card">
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.map(c => (
                <tr key={c.id} className="hover:bg-slate-50/60 transition-colors group">
                  ...
                  <td className="px-4 py-3.5">
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-md ${priorityColor[c.priority]}`}>
                      {c.priority}
                    </span>
                  </td>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100 bg-slate-50/50">
          ...
          <div className="flex items-center gap-1">
            {[1, 2, 3].map(p => (
              <button key={p} className={`w-7 h-7 text-xs rounded-md transition-colors ${p === 1 ? 'bg-brand-600 text-white' : 'text-slate-600 hover:bg-slate-200'}`}>{p}</button>
            ))}
          </div>
        </div>
      </div>
